...
#include "utils/log.h"
    ...
    // ;/?:@&=+$,[] 和 -_.!~*'() 和 0-9 和 a-z 和 A-Z
    std::bitset <MAX_BIT_SIZE> g_ruleUrlc;

    // ;/ 和 :@&=+$, 和 -_.!~*'() 和 0-9 和 a-z 和 A-Z
...
// 考虑合并AnalysisFragment 和 AnalysisQuery 为一个
...
bool URI::AnalysisIPV4()
{
    ...
    std::regex hostname("([a-zA-Z0-9]([a-zA-Z0-9\\-]*[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,6}");
...
{
    std::string str = data_.substr(1, data_.size() - 2);
    std::regex ipv6("^((?:[0-9A-Fa-f]{1,4}(?::[0-9A-Fa-f]{1,4})*)?)::((?:[0-9A-Fa-f]{1,4}(?::[0-9A-Fa-f]{1,4})*)?)$");
